...
import { addTodo, addTodoAsync, addTodoIfEmpty } from './helpers/action-creators'
...
describe('applyMiddleware', () => {
  ...
  test('passes recursive dispatches through the middleware chain', () => {
    function test(spyOnMethods) {
      return () => next => action => {
        spyOnMethods(action)
        return next(action)
  ...
  test('works with thunk middleware', done => {
    // @ts-ignore

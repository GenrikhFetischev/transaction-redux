...
describe("Utils", () => {
  describe("combineReducers", () => {
    ...
    test("warns if a reducer prop is undefined", () => {
    ...
    test("throws an error if a reducer returns undefined handling an action", () => {
      const reducer = combineReducers({
        counter(state: number = 0, action) {
    ...
    test("throws an error on first call if a reducer returns undefined initializing", () => {
      const reducer = combineReducers({
    ...
    test("catches error thrown in reducer when initializing and re-throw", () => {
      const reducer = combineReducers({
        throwingReducer() {
    ...
    test("allows a symbol to be used as an action type", () => {
    ...
    test("maintains referential equality if the reducers it is combining do", () => {
      const reducer = combineReducers({
        child1(state = {}) {
          return state;
        },
    ...
    test("does not have referential equality if one of the reducers changes something", () => {
      const reducer = combineReducers({
        child1(state = {}) {
          return state;
        },
    ...
    test("throws an error on first call if a reducer attempts to handle a private action", () => {
      const reducer = combineReducers({
        counter(state: number, action) {
          switch (action.type) {
            case "increment":
    ...
    test("warns if no reducers are passed to combineReducers", () => {
      const preSpy = console.error;
      const spy = jest.fn();
      console.error = spy;
    ...
    test("warns if input state does not match reducer shape", () => {
      const preSpy = console.error;
      const spy = jest.fn();
    ...
    test("only warns for unexpected keys once", () => {
      const preSpy = console.error;
      const spy = jest.fn();
      console.error = spy;
    ...
    describe("With Replace Reducers", function() {
      ...
      test("should return an updated state when additional reducers are passed to combineReducers", function() {
      ...
      test("should return an updated state when reducers passed to combineReducers are changed", function() {
      ...
      test("should return the same state when reducers passed to combineReducers not changed", function() {
        const originalCompositeReducer = combineReducers({ foo, bar });
        const store = createStore(originalCompositeReducer);
      ...
      // TODO make it works
      test.skip("should return an updated state when one of more reducers passed to the combineReducers are removed", function() {
        const originalCompositeReducer = combineReducers({ foo, bar });
        const store = createStore(originalCompositeReducer);
